...
public class SettingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String percent_raw = request.getParameter("percent");
        String limit_raw = request.getParameter("limit");
        BookingDAO bk = new BookingDAO();
        double p = Double.parseDouble(percent_raw);
        String ms = "";
        String error = "";
        ...
        if (flag1 && flag2){
            ...
            error += "Update Limit Booking Failed!";
        } else if (!flag1 && flag2) {
            error += "Update Deposit Percent Failed!";
        } else {
            error += "Update Both Failed!";
        }
        
        request.setAttribute("error", error);
        request.setAttribute("ms", ms);
        request.getRequestDispatcher("coWeb-setting.jsp").forward(request, response);
    }
